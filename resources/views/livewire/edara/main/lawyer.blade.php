<div>
    ...
    <main class="md:mr-72 p-4 md:p-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 space-y-4 md:space-y-0">
            <h1 class="text-2xl font-bold">إدارة المحامون</h1>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-4 md:p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse ($lawyers as $lawyer)
                    @php
                        $status = $this->getStatus($lawyer);
                    @endphp
                    <div class="bg-white dark:bg-gray-700 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-600">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-center">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($lawyer->name) }}&background=random" 
                                     alt="Profile" 
                                     class="w-12 h-12 rounded-full ml-3">
                                <div>
                                    <h3 class="font-medium text-lg">{{ $lawyer->name }}</h3>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $lawyer->email }}</p>
                                </div>
                            </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center text-xs md:text-sm">
                                ...
                                <span>فعال من: {{ \Carbon\Carbon::parse($lawyer->active_at)->format('Y-m-d') }}</span>
                            </div>
                            <div class="flex items-center text-xs md:text-sm">
                                <svg class="h-4 w-4 md:h-5 md:w-5 ml-2 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                </svg>
                                <span>ينتهي في: {{ \Carbon\Carbon::parse($lawyer->expired_at)->format('Y-m-d') }}</span>
                            </div>
                            @php
                                $clientCount = \App\Models\User::where('lawyer_id', $lawyer->id)->count();
                            @endphp
                            <div class="flex items-center text-xs md:text-sm">
                                <svg class="h-4 w-4 md:h-5 md:w-5 ml-2 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600 flex justify-end space-x-2">
                            ...
                            @if(($lawyer->is_active))
                                <button 
                                    wire:click="deactivate({{ $lawyer->id }})"
                ...
                @empty
                    <div class="col-span-3 text-center py-8">
                        <p class="text-gray-500 dark:text-gray-400">لا يوجد محامون مسجلون</p>
                    </div>
                @endforelse
            </div>
        ...
        >
            ...
            >
                <h2 class="text-xl font-bold mb-4">
                    {{ $lawyerId ? 'تعديل بيانات المحامي' : 'إضافة محامي جديد' }}
                </h2>
